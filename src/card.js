...
// человек сохраняет и кидает в чат. Не трогает DOM страницы и не
// запрашивает данные сама: числа и даты ей передаёт src/app.js в объекте
// data (см. shape ниже), сама она только рисует.
//
...
// три и что они разные, а не только итоговый результат. Личный результат
// остаётся заметным — средняя колонка подсвечена плашкой фона и меткой
// «твой выбор», плюс только у неё есть строка с суммой в долларах.
//
...
(function () {
  ...
  function fitFontSize(ctx, text, maxWidth, weight, maxSize, minSize) {
    ...
    // подстраховки цикл может проскочить minSize на 1-2px вниз: например,
    // у подписи даты 30→20 разница 10 не делится на 4 без остатка.
  ...
  function renderShareCard(canvas, data) {
    ...
    // оно есть на самом деле, а на карточку попадают именно такие
    // оговорки — те, что льстят нашей же цифре, если промолчать.
