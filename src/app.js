...
(function () {
  ...
  // язык определяется адресом, а нужную версию поиск подбирает сам по
  // hreflang в разметке. Автоперехода по языку браузера нет сознательно:
  // он дублировал бы hreflang, а Googlebot заходит как англоязычный
  // браузер и принял бы корень за редирект, так что русская версия могла
  // не попасть в индекс. Остался только переход по явному параметру
  // ?lang=en / ?lang=ru: так работают старые ссылки и кнопка-переключатель.
  const lang = document.documentElement.lang;
  const param = new URLSearchParams(location.search).get("lang");
  if ((param === "en" || param === "ru") && param !== lang) {
    location.replace(new URL(param === "en" ? "en/" : "../", location.href));
    return;
  }
